 ...
 * @returns {Object}
 */
export function mscodeDeepClone(source) {
  if (!source && typeof source !== 'object') {
    throw new Error('error arguments', 'mscodeDeepClone')
  }
  const targetObj = source.constructor === Array ? [] : {} // NOSONAR
  Object.keys(source).forEach(keys => {
    if (source[keys] && typeof source[keys] === 'object') {
      targetObj[keys] = mscodeDeepClone(source[keys])
    } else {
...
function parse(str) {
  JSON.parse(str, (k, v) => {
    if (v.indexOf && v.indexOf('function') > -1) {
      return eval(`(${v})`) /* eslint no-eval: 0 */
    }
    return v
  })
}
